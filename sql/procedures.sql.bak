...
BEGIN  
	...
	BEGIN
	RAISERROR('ce type  ne peux pas etre supprimé ', 9, 1);
...
-------------------------------------------------------
/* IBDR 2013 - Groupe SAR                            */
/* Procédure de supression d'un abonnement           */
/* Auteur  : RAHMOUN Imane - SAR                     */
/* Testeur : RAHMOUN Imane - SAR                     */
-------------------------------------------------------
IF OBJECT_ID ( 'dbo.abonnement_supprimer', 'P' ) IS NOT NULL 
    DROP PROCEDURE dbo.abonnement_supprimer;
GO
CREATE PROCEDURE [dbo].[abonnement_supprimer]
@Id INT 
AS 
BEGIN  

	DELETE dbo.Abonnement 
	FROM dbo.Abonnement 
	WHERE @Id=Id
